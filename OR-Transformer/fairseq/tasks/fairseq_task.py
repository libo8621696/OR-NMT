...
class FairseqTask(object):
    ...
    def train_step(self, sample, model, criterion, optimizer, epoch_itr=None, num_updates=None, ignore_grad=False):
        """
        ...
        loss, sample_size, logging_output = criterion(
            model, sample,
            epoch_itr=epoch_itr, num_updates=num_updates,
            bos_idx=self.target_dictionary.bos())
        if ignore_grad:
            loss *= 0
    ...
    def valid_step(self, sample, model, criterion):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, sample, is_test=True)
        return loss, sample_size, logging_output
